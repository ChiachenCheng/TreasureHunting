...
@bp.route("/<username>/register", methods=['GET'])
def register_route(username):
    status = register(username)
...
def get_my_treasure_route(username):
    player = get_my_treasure(username)
...
@bp.route("/<username>/treasures/more", methods=['GET'])
